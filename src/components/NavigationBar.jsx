...
export default function NavigationBar() {
        ...
        (async () => {
            try {
                if (auth.token) {
                    ...
                    json.status
                        ? json.status === "success"
                            ? dispatch({ type: "SET_AUTH", payload: { ...auth, isLogin: true, loginData: json, token: auth.token } })
                            : dispatch({ type: "SET_AUTH", payload: { ...auth, isLogin: false, loginData: {}, token: "" } })
    ...
    async function handleLogout(e) {
        e.preventDefault();
        try {
            const response = await fetch(API_BASE_URL + "/api/auth/logout?token=" + auth.token);
            if (!response.ok) {
                throw new Error("Network response was not ok");
            }
            localStorage.removeItem("token");
    ...
    return (
        <>
            <nav className="navbar navbar-expand-lg bg-body-tertiary pk-navigation-bar sticky-top">
                <div className="container-fluid">
                    <a className="navbar-brand" href="#">
                        <img className="pk-ball-logo" src="../img/pokeball.png" alt="" />
                        <img className="pk-logo" src="../img/pokemon-logo.png" alt="" />
                        <img className="pk-logo" style={{ width: "70px" }} src="../img/sgt-logo.png" alt="" />
                    </a>
                    <button
                                        ...
                                        {!auth.isLogin ? (
                                            ...
                                            <ul className="dropdown-menu text-center">
                                                <li>
                                                    <Link to={"/myPokemon"}>
                                                        {" "}
                                                        <button style={{margin:'10px', backgroundColor:'blue'}}>Go to My Pokemon</button>
                                                    </Link>
                                                </li>
                                                <li>
